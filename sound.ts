...
namespace sound
{
    ...
    //% useEnumVal=1 color="#FFFFFF" colorSecondary="#FFFFFF" shim=TD_ID
    //% weight=2 blockGap=8
    ...
    /**
     * Converts a frequency into a gamut suitable for using with
     * playtone and ringtone messages (see message format above).
     */
    export function _toneForNote(note: Note): number {
        let gamut = 0;
        note = findNearestNote(note)
        switch(note)
        {
            case Note.C3: gamut = 1; break;
            case Note.D3: gamut = 2; break;
            case Note.E3: gamut = 3; break;
            case Note.F3: gamut = 4; break;
            case Note.G3: gamut = 5; break;
            case Note.A3: gamut = 6; break;
            case Note.B3: gamut = 7; break;

            case Note.C4: gamut = 8; break;
            case Note.D4: gamut = 9; break;
            case Note.E4: gamut = 10; break;
            case Note.F4: gamut = 11; break;
            case Note.G4: gamut = 12; break;
            case Note.A4: gamut = 13; break;
            case Note.B4: gamut = 14; break;

            case Note.C5: gamut = 15; break;
            case Note.D5: gamut = 16; break;
            case Note.E5: gamut = 17; break;
            case Note.F5: gamut = 18; break;
            case Note.G5: gamut = 19; break;
            case Note.A5: gamut = 20; break;
            case Note.B5: gamut = 21; break;

            case Note.CSharp3: gamut = 22; break;
            case Note.Eb3: gamut = 23; break;
            case Note.FSharp3: gamut = 24; break;
            case Note.GSharp3: gamut = 25; break;
            case Note.Bb3: gamut = 26; break;

            case Note.CSharp4: gamut = 27; break;
            case Note.Eb4: gamut = 28; break;
            case Note.FSharp4: gamut = 29; break;
            case Note.GSharp4: gamut = 30; break;
            case Note.Bb4: gamut = 31; break;

            case Note.CSharp5: gamut = 32; break;
            case Note.Eb5: gamut = 33; break;
            case Note.FSharp5: gamut = 34; break;
            case Note.GSharp5: gamut = 35; break;
            case Note.Bb5: gamut = 36; break;
    ...
    function findNearestNote(note: number) {
        if (note < Notes[0]) {
            return Notes[0]
        }
        ...
        while (lo <= hi) {
            // let mid = Math.floor((lo + hi)/2) // Not api support "Math.floor()"
            let mid = (lo + hi)/2
            if (note < Notes[mid]) {
                hi = mid-1
            } else if (note > Notes[mid]) {
                lo = mid+1
            } else {
                return Notes[mid]
            }
        }
        return (Notes[lo] - note) < (note - Notes[hi]) ? Notes[lo] : Notes[hi]
    }
